...
System.register("chunks:///_virtual/PopWarehouse.ts", ['./rollupPluginModLoBabelHelpers.js', 'cc', './BaseView.ts', './IconPrefab.ts', './app.ts', './BattleUtil.ts', './cc-comp-list-view.ts', './export.type.ts', './ListView.ts'], function (exports) {
  var _applyDecoratedDescriptor, _inheritsLoose, _initializerDefineProperty, _assertThisInitialized, cclegacy, _decorator, Node, Button, BaseView, IconPrefab, app, BattleUtil, ItemType, ListView;
  return {
    ...
    execute: function () {
      ...
      var PopWarehouse = exports('PopWarehouse', (_dec = ccclass('PopWarehouse'), _dec2 = property(ListView), _dec3 = property(ListView), _dec4 = property(Node), _dec(_class = (_class2 = /*#__PURE__*/function (_BaseView) {
        ...
        _proto.onBagRenderEvent = function onBagRenderEvent(item, index) {
          var _this2 = this;
          var data = app.store.player.bag.get(ItemType.weapon)[index];
          item.getComponent(IconPrefab).updateUI(data, function () {
            // 背包的道具放到仓库
            app.store.player.addItemToWarehouse(BattleUtil.deepClone(data));
            //删除背包的道具
            app.store.player.removeItemFromBag(index, ItemType.weapon);
            // 更新UI
            _this2.updateUI();
          });
        };
